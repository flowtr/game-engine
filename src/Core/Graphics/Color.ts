export type ColorSerialization<E = Record<string, unknown>> = E & {
    r?: number;
    g?: number;
    b?: number;
    a?: number;
...
export class Color {
     ...
     */
    public static fromJson(json: ColorSerialization): Color {
        let c = new Color();
        if (json.r !== undefined) c.r = Number(json.r);

        if (json.g !== undefined) c.g = Number(json.g);

        if (json.b !== undefined) c.b = Number(json.b);

        if (json.a !== undefined) c.a = Number(json.a);
