...
import { Color, ColorSerialization } from "./Color";
...
export class MaterialConfig {
    ...
    public static fromJson(json: {
        ...
        diffuse?: string;
        specular?: string;
        tint?: ColorSerialization;
    }): MaterialConfig {
        let config = new MaterialConfig();
        if (json.name !== undefined) config.name = String(json.name);

        if (json.shader !== undefined) config.shader = String(json.shader);

        if (json.diffuse !== undefined) config.diffuse = String(json.diffuse);

        if (json.specular !== undefined)
            config.specular = String(json.specular);

        if (json.tint !== undefined) config.tint = Color.fromJson(json.tint);
        else config.tint = Color.white();
...
export class MaterialManager {
    ...
    public static registerMaterial(materialConfig: MaterialConfig): void {
        if (
            MaterialManager._materialConfigs[materialConfig.name] === undefined
        ) {
            MaterialManager._materialConfigs[
                materialConfig.name
            ] = materialConfig;
        }
    }
    ...
    public static getMaterial(materialName: string): Material {
        if (MaterialManager._materials[materialName] === undefined) {
            ...
            if (MaterialManager._materialConfigs[materialName] !== undefined) {
                let mat = Material.FromConfig(
                    MaterialManager._materialConfigs[materialName]
                );
                MaterialManager._materials[
                    materialName
                ] = new MaterialReferenceNode(mat);
