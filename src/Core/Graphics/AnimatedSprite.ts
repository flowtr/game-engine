...
class UVInfo {
    public min: Tensor<Vector<2>>;
    public max: Tensor<Vector<2>>;

    public constructor(min: Tensor<Vector<2>>, max: Tensor<Vector<2>>) {
        this.min = min;
        this.max = max;
    }
}
...
export class AnimatedSprite extends Sprite implements IMessageHandler {
    ...
    public update(time: number): void {
        ...
        if (this._currentTime > this._frameTime) {
            ...
            this._vertices[0].texCoords = this._frameUVs[frameUVs].min.clone();
            this._vertices[1].texCoords = Tensor.from(
                this._frameUVs[frameUVs].min.x,
                this._frameUVs[frameUVs].max.y
            );
            this._vertices[2].texCoords = this._frameUVs[frameUVs].max.clone();
            this._vertices[3].texCoords = this._frameUVs[frameUVs].max.clone();
            this._vertices[4].texCoords = Tensor.from(
                this._frameUVs[frameUVs].max.x,
                this._frameUVs[frameUVs].min.y
            );
            this._vertices[5].texCoords = this._frameUVs[frameUVs].min.clone();
    ...
    private calculateUVs(): void {
        ...
        for (let i = 0; i < this._frameCount; ++i) {
            ...
            let min: Tensor<Vector<2>> = Tensor.from<Vector<2>>(u, v);
            ...
            let vMax =
                ...
                this._assetHeight;
            let max: Tensor<Vector<2>> = Tensor.from<Vector<2>>(uMax, vMax);
