...
export class Rectangle2D implements IShape2D {
     ...
     */
    public setFromJson(
        json: IShape2DSerialization<{ width: number; height: number }>
    ): void {
    ...
    public pointInShape(point: Tensor<Vector<2>>): boolean {
        ...
        let y =
            this.height < 0 ? this.position.y - this.height : this.position.y;
